...
hide_status_on() {
  ...
  local check_plugin_status="$(cat $XDG_CONFIG_HOME/tmux/tmux.conf |awk '/^[ \t]*set(-option)? +-g +@plugin/ { gsub(/'\''/,""); gsub(/'\"'/,""); print $4 }' | grep 'tmux-plugins/tmux-continuum')"

  if [[ -n "${check_plugin_status}" ]]; then
    local plugin_script="$(readlink -m $XDG_DATA_HOME/tmux/plugins/tmux-continuum/scripts/continuum_save.sh)"
